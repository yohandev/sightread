...
/// an array of keys, represented as MIDI (note, velocity) bytes and
/// converted to [Key]s and [Velocity]s on the fly if needed
///
/// [Key]: Key
/// [Velocity]: Velocity
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash)]
pub struct Keys<'a>(&'a [(MidiKey, Velocity)]);
...
impl<'a> Keys<'a>
{
    /// create a new array of keys from a reference
    pub fn new(keys: &'a [(MidiKey, Velocity)]) -> Self
    {
        Self(keys)
    }

    /// get the `Key` representation of these MIDI keys
    pub fn into_keys(&self)// -> impl Iterator<Item = (Key, Velocity)>
    {
        todo!()
    }

    /// apply the keys in self and their velocities to the given keyboard
    pub fn apply(&self, keyboard: &mut Keyboard)
    {
        for (key, vel) in self.0
        {
            keyboard[*key] = *vel;
        }
    }
...
impl TryFrom<MidiKey> for Key
{
    type Error = ();

    fn try_from(value: MidiKey) -> Result<Self, Self::Error>
    {
        todo!()
    }
}

impl<'a> Index<usize> for Keys<'a>
{
    type Output = (MidiKey, Velocity);
...
impl IndexMut<Key> for Keyboard
{
    ...
    {
        &mut self.state[index.ordinal()]
    }
}

impl Index<MidiKey> for Keyboard
{
    type Output = Velocity;

    fn index(&self, index: MidiKey) -> &Self::Output
    {
        &self.state[index.0 as usize - 21]
    }
}

impl IndexMut<MidiKey> for Keyboard
{
    fn index_mut(&mut self, index: MidiKey) -> &mut Self::Output
    {
        &mut self.state[index.0 as usize - 21]
    }
}
